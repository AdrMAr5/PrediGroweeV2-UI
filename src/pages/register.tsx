    ...
    return (
        <AuthPagesLayout>
            <Card sx={{ maxWidth: 450, width: '100%' }}>
                ...
                <CardContent>
                    <Box component="form" noValidate sx={{ mt: 1 }} onSubmit={(event)=>{
                        event.preventDefault();
                        const email = (event.target as any).email.value;
                        const password = (event.target as any).password.value;
                        const retypePassword = (event.target as any).retypePassword.value;
                        console.log(email, password, retypePassword);
                        usersClient.register(email, password).then((response)=>{
